...
\centering\includegraphics[height=0.66\textheight]{data_composition.png}\\
...
\centering\includegraphics[height=0.70\textheight]{umap_panel.png}\\
...
\small
\textit{Two tracks:} \hl{(A) representation} (trainable encoder) = SIGReg + Pathway;
\hl{(B) world model} (\textbf{frozen} encoder $\Rightarrow$ no collapse, no anti-collapse term) = PerturbationSignature + Pathway.\\[3pt]
\begin{tcolorbox}[colback=secondary!6,colframe=primary,title={1. SIGReg (anti-collapse; track A; our choice vs GeneJEPA VICReg)}]
...
% ---------------- 6b. COLLAPSE ABLATION ----------------
\begin{frame}{The collapse we fought --- SIGReg keeps the encoder alive}
\centering\includegraphics[height=0.62\textheight]{collapse.png}\\
{\footnotesize Same cell JEPA on PBMC3k, \hl{with vs without} the anti-collapse term.
Without it the embedding \hl{std $\to 0.002$} (collapse), effective rank $\to 1$, and the
cell-type probe drops to \hl{0.43} ($\approx$ chance). With SIGReg: \hl{std $\approx 1.1$, probe $0.94$}.}
\end{frame}
...
\centering\includegraphics[width=\linewidth]{fig_skill.png}
\end{column}
