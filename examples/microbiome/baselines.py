...
def _probe(Xt, at, yt, Xv, av, yv):
    ...
    out = {}
    age_pred = Ridge(alpha=1.0).fit(Xt, at).predict(Xv)
    out["age_r2"] = float(r2_score(av, age_pred))
    out["age_r"] = float(pearsonr(av, age_pred)[0]) if len(av) > 2 else float("nan")
...
def _probe_rf(Xt, at, yt, Xv, av, yv, seed=0):
    """Random Forest on raw features — the standard non-parametric published baseline
    (MetAML, SSL-microbiome papers all report RF as the go-to supervised reference)."""
    from scipy.stats import pearsonr
    ...
    age_pred = reg.fit(Xt, at).predict(Xv)
    out["age_r2"] = float(r2_score(av, age_pred))
    out["age_r"] = float(pearsonr(av, age_pred)[0]) if len(av) > 2 else float("nan")
    if len(np.unique(yt)) == 2 and len(np.unique(yv)) == 2:
...
def main():
    ...
    rows[f"random encoder (n={len(args.rand_seeds)})"] = {
        ...
        "age_r": (float(np.nanmean(rand.get("age_r", [float("nan")]))),
                  float(np.nanstd(rand.get("age_r", [float("nan")])))),
        "t1d_auroc": (float(np.nanmean(rand["t1d_auroc"])), float(np.nanstd(rand["t1d_auroc"]))),
    }

    # ---- 5b. RF on raw mean-ProkBERT (published MetAML / SSL-microbiome reference) --
    Xt, at, yt = _gather_raw(train_loader)
    Xv, av, yv = _gather_raw(val_loader)
    rows["RF on raw (MetAML ref)"] = _probe_rf(Xt, at, yt, Xv, av, yv)
    ...
    print(f"{'representation':<26}{'age_r2':>14}{'age_r':>12}{'t1d_auroc':>14}")
    print("-" * 66)
    for name, m in rows.items():
        age_r = m.get("age_r", float("nan"))
        print(f"{name:<26}{_fmt(m['age_r2']):>14}{_fmt(age_r):>12}{_fmt(m['t1d_auroc']):>14}")
    if not ckpts:
        print("\n(no --ckpt given: 'JEPA (trained)' row + dynamics table omitted. "
              "Pass --ckpt <path> [<path> ...] to add them.)")
    print("\nRead: age_r2 = host-age R² | age_r = Pearson r (SSL-microbiome protocol) | "
