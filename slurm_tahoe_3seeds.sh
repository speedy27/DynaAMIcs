...
#SBATCH --output=tahoe_seed%a_%j.out
#SBATCH --error=tahoe_seed%a_%j.err
#SBATCH --array=0-2
...
# Shared sweep root across array tasks (timestamp from job submission)
SWEEP_TAG="${SWEEP_TAG:-$(date +%Y%m%d)_$SLURM_ARRAY_JOB_ID}"
OUT_ROOT="${EBJEPA_CKPTS:-$REPO/checkpoints}/tahoe_sweep_${SWEEP_TAG}"
RUN_DIR="$OUT_ROOT/seed${SEED}_sigreg"
mkdir -p "$RUN_DIR"
echo "=== output: $RUN_DIR ==="
...
EBJEPA_CKPTS="$RUN_DIR" uv run --project "$REPO" python -m examples.tahoe.main \
    --fname examples/tahoe/cfgs/train.yaml \
    meta.seed=$SEED \
